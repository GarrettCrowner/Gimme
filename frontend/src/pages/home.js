...
export async function renderHome(app, navigate) {
  ...
  try {
    ...
    if (active.length) {
      ...
      active.forEach(r => wrap.appendChild(roundCard(r, navigate, true)));
    }

    if (completed.length) {
      wrap.appendChild(el("h2", { style: "margin-top:1.25rem" }, "Past Rounds"));
      completed.slice(0, 10).forEach(r => wrap.appendChild(roundCard(r, navigate, false)));
    }
...
function roundCard(round, navigate, isActive) {
